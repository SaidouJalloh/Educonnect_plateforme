...
// code precedent marche bien mais avec des données fictif
import { useState, useEffect, useCallback } from "react";
import Navigation from "@/components/Navigation";
import OpportunityCard from "@/components/OpportunityCard";
import { Input } from "@/components/ui/input";
...
import { Search, RefreshCw, AlertCircle, Inbox, Sparkles } from "lucide-react";
import { useToast } from "@/hooks/use-toast";

// 💡 IMPORTS CRUCIAUX POUR TON IA ET SUPABASE
...
const Opportunities = () => {
  const { toast } = useToast();
  const { user } = useAuthContext(); // Récupération de l'étudiant connecté
  ...
  const loadOpportunities = useCallback(async (showRefreshToast = false) => {
    ...
    try {
      ...
      if (user) {
        // 🚀 L'UTILISATEUR EST CONNECTÉ : ON APPELLE TON IA PYTHON !
        ...
        if (!response.ok) {
          throw new Error("Erreur de connexion avec le moteur IA.");
        }

        const aiData = await response.json();
        // On fusionne les bourses et les formations recommandées
      ...
      setFilteredOpportunities(dataToSet);
    ...
    if (searchQuery) {
      ...
      filtered = filtered.filter(
        (opp) =>
          opp.titre.toLowerCase().includes(query) ||
          opp.description.toLowerCase().includes(query) ||
          (opp.pays && opp.pays.toLowerCase().includes(query))
      );
    }
    ...
    <div className="min-h-screen bg-gradient-subtle">
      ...
      <main className="pt-16">
        <div className="container mx-auto px-4 py-12">
          ...
          <div className="max-w-3xl mx-auto text-center mb-12">
            <h1 className="text-4xl md:text-5xl font-bold mb-4">
              {user ? "Vos Opportunités " : "Opportunités "}
              <span className="bg-gradient-primary bg-clip-text text-transparent flex justify-center items-center gap-2 mt-2">
                {user ? "Recommandées par l'IA" : "Disponibles"}
                {user && <Sparkles className="w-8 h-8 text-primary" />}
              </span>
            </h1>
            <p className="text-lg text-muted-foreground mt-4">
              {user
                ? "Notre modèle IA a analysé votre profil et sélectionné ces opportunités sur mesure."
                : "Connecte-toi pour que notre IA analyse ton profil et te recommande les meilleures opportunités."}
            </p>
            {!loading && (
              <p className="text-sm text-muted-foreground mt-2">
                {filteredOpportunities.length} opportunité{filteredOpportunities.length > 1 ? "s" : ""} trouvée{filteredOpportunities.length > 1 ? "s" : ""}
          ...
          <div className="max-w-5xl mx-auto mb-8 flex flex-col md:flex-row gap-4">
            ...
            <Button variant="outline" className="gap-2 rounded-xl" onClick={handleRefresh} disabled={isRefreshing}>
              <RefreshCw className={`w-4 h-4 ${isRefreshing ? "animate-spin" : ""}`} />
              Rafraîchir
            </Button>
          </div>
          ...
          {!loading && paginatedOpportunities.length > 0 && (
            <div className="max-w-5xl mx-auto">
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                {paginatedOpportunities.map((opportunity, index) => (
                  <div key={opportunity.id} className="animate-fade-in flex flex-col" style={{ animationDelay: `${index * 50}ms` }}>
                    <OpportunityCard
                      title={opportunity.titre}
                      description={opportunity.description}
                      category={opportunity.type}
                      location={opportunity.pays || "En ligne"}
                      deadline={opportunity.date_limite || "Non spécifié"}
                      link={opportunity.lien || "#"}
                    />
                    {/* 🚀 L'AFFICHAGE DU MATCH IA EST ICI */}
                    {opportunity.score_ia && (
                      <div className="mt-2 flex justify-end">
                        <span className="inline-flex items-center gap-1 bg-green-100 text-green-800 text-xs px-3 py-1.5 rounded-full font-bold shadow-sm border border-green-200">
                          <Sparkles className="w-3.5 h-3.5" />
                          Match IA : {Math.round(opportunity.score_ia * 100)}%
                        </span>
                      </div>
                    )}
                  </div>
